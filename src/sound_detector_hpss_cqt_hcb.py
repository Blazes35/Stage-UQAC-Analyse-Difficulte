...
def find_audio_patterns_hpss_cqt(main_audio_path, template_path, threshold=0.55, min_dist_seconds=3.0):
    ...
    # --- THE NEW TRICK: Aggressive High-Contrast Clipping ---
    # We find the loudest point in the audio, and chop off everything that is
    # more than 15 decibels quieter than it. This deletes the "background hum".
    print("Applying high-contrast masks...")
    C_main_db[C_main_db < (np.max(C_main_db) - 15)] = np.min(C_main_db)
    C_template_db[C_template_db < (np.max(C_template_db) - 15)] = np.min(C_template_db)
    # --------------------------------------------------------
